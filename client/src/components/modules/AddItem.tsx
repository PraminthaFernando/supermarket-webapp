...
function AddItem() {
  ...
  const handleChange = (event: any) => {
    ...
    if (customerIDs.includes(selectedValue)) {
      setValidationMessage("This Item ID already exists."); // Log validation message if needed
    } else {
      setValidationMessage("");
      setSN(selectedValue); // Set the customer ID if not found
    }
  };
  ...
  const handleCreate = async (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (validationMessage) {
      ...
      try {
        await axios.put("http://localhost:8000/item/create", {
          ID: SN,
          Name: name,
        });
        ...
        setMsg(`Item ${name}`);
        {
          setId(""), setName(""), setValidationMessage("");
        }
      } catch (err) {
        console.log(err);
      }
